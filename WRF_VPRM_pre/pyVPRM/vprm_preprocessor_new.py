...
# Coose landcover file:

# LC100
# landcover_file = "E000N60_PROBAV_LC100_global_v3.0.1_2019-nrt_Discrete-Classification-map_EPSG-4326.tif"
# remapping_file = "copernicus_land_cover.yaml"

# CORINE
landcover_file = "modified_for_VPRM_U2018_CLC2018_V2020_20u1.tif"
remapping_file = "copernicus_land_cover_corine_mapping.yaml"
...
for c, i in enumerate(hvs):
    ...
    new_inst = vprm(
        vprm_config_path=os.path.join(
            pyVPRM.__path__[0], "vprm_configs/" + remapping_file
        ),
        n_cpus=args.n_cpus,
    )
...
if os.path.exists(veg_file):
    ...
    if copernicus_data_path is not None:
        ...
        for i, c in enumerate(
            glob.glob(os.path.join(copernicus_data_path, landcover_file))
        ):
            print(c)
