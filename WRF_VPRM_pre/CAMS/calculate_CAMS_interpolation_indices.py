...
print(
    "a selected CAMS forecast netcdf file. A major overhaul of matlab script from Julia Marshall"
)
...
filein = os.path.join(cams_path, "ghg-reanalysis_lat_43_51_lon_5_17_2020.nc")
...
for current_domain_idx in requested_domains:
    ...
    for x1 in range(xlat.shape[1]):
        ...
        for x2 in range(xlat.shape[0]):
            testdist = short_dist(
                [xlat[x2, x1], xlong[x2, x1]],
                np.concatenate(
                    (mesh_lat.reshape(-1, 1), mesh_lon.reshape(-1, 1)), axis=1
                ),
            )
...
np.savez(
    output_file + "interp_indices.txt", **{var: globals()[var] for var in variables}
)
