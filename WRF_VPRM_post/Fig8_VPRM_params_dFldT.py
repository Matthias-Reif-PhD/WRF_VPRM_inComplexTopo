"""
Fig8_VPRM_params_dFldT.py
==============================================
Fig. 8 (fig:tempderivative).
This module plots:
...
# Canonical palette, in the key order of pft_parameters below
# (Evergreen / Deciduous / Mixed Forest, Cropland, Grassland).
import sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from topt_box import PFT_COLORS as _CANON

PLOT_COLORS = [_CANON[c] for c in ("ENF", "DBF", "MF", "CRO", "GRA")]
...
def plot_gpp_and_reco(
    ...
    plt.xlabel(r"T$_\mathrm{2m}$ [°C]", fontsize=16)
    plt.ylabel(r"GPP and R$_{\mathrm{eco}}$ [$\mu$mol m$^{-2}$ s$^{-1}$]", fontsize=16)
...
def plot_dGPP_dT_and_dRECO_dT(
    ...
    plt.ylabel(
        ...
        fontsize=12,  # long label: at 16 pt it overran the 4.2 in canvas and clipped
    )
    plt.tick_params(labelsize=14)
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.tight_layout()
